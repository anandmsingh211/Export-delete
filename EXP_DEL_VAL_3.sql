...
SPOOL /backup/exa_ps/dba/PS_ARCHIVAL/LOG/EXP_DEL_VAL_3.LOG
SET ECHO OFF
SET AUTOCOMMIT OFF
SET FEEDBACK OFF
SET TIME ON
...
-- **********************************************************************
-- SQL Script File Name: EXP_DEL_VAL_3.sql
-- **********************************************************************
-- 
-- Description:
...
BEGIN
  ...
  BEGIN
    ...
    WHILE K IS NOT NULL LOOP
      ...
      IF V_ACTUAL_TOTAL = V_EXPECTED_TOTAL THEN
         ...
         -- NEW: Record only tables that actually had any deletes
  ...
  ------------------------------------------------------------------------------
  -- BLOCK: POST-DELETE TABLE MOVE + INDEX REBUILD (ONLY TABLES WITH DELETES)
  ...
  BEGIN
    ...
    WHILE TBL_KEY IS NOT NULL LOOP
      TBL := TBL_KEY;  -- bare table name like PS_XXXX

      DBMS_OUTPUT.PUT_LINE('Starting maintenance for table: ' || TBL);

      ------------------------------------------------------------------
      -- TABLE MOVE
      ------------------------------------------------------------------
      BEGIN
        EXECUTE IMMEDIATE 'ALTER TABLE SYSADM.PS_' || TBL || ' ENABLE ROW MOVEMENT';
        EXECUTE IMMEDIATE 'ALTER TABLE SYSADM.PS_' || TBL || ' MOVE PARALLEL 4';
        EXECUTE IMMEDIATE 'ALTER TABLE SYSADM.PS_' || TBL || ' DISABLE ROW MOVEMENT';
        DBMS_OUTPUT.PUT_LINE('  Table moved successfully: ' || TBL);
      EXCEPTION
        WHEN OTHERS THEN
          DBMS_OUTPUT.PUT_LINE('  WARNING: Table move failed for PS_' || TBL || ' — ' || SQLERRM);
      END;
