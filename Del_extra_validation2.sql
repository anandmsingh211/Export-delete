...
spool c:\temp\Del_extra_validation2.LOG
...
-- **********************************************************************
-- SQL Script File Name: Del_extra_validation2.SQL
-- **********************************************************************
-- KEYWORD: ~XX
-- **********************************************************************
--
...
BEGIN
  ...
  BEGIN
    -- 6.a: Resolve directory path (using ALL_DIRECTORIES for non-DBA users)
    BEGIN
    ...
    -- 6.c: Open the file
    F_LOG := UTL_FILE.FOPEN(UPPER('&P_LOG_DIR'), '&P_LOG_FILE', 'R', 32767);
    ...
    LOOP
      ...
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          -- EOF reached: close and break cleanly
          UTL_FILE.FCLOSE(F_LOG);
          EXIT READ_LOOP;
        WHEN UTL_FILE.INVALID_OPERATION OR UTL_FILE.READ_ERROR THEN
          -- Close handle and raise a clear error (e.g., line too long)
          BEGIN
            UTL_FILE.FCLOSE(F_LOG);
  ...
  /* =======================================================================
     ...
     NOTE: Iteration over associative array keys (INDEX BY VARCHAR2)
     must use FIRST/NEXT pattern.
     ======================================================================= */
  DECLARE
    K VARCHAR2(400);
  BEGIN
    ...
    WHILE K IS NOT NULL LOOP
      -- Check if this exported key was deleted
