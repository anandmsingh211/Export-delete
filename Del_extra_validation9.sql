...
spool /backup/exa_ps/dba/PS_Archival/LOG/Del_extra_validation9.LOG
SET ECHO OFF
SET AUTOCOMMIT OFF
SET FEEDBACK OFF
SET TIME ON
...
-- **********************************************************************
-- SQL Script File Name: Del_extra_validation9.SQL
-- **********************************************************************
-- 
-- Description:
...
DEFINE P_LOG_DIR    = '&4'
DEFINE P_LOG_FILE   = '&5'
DEFINE P_TEMPLATE_CLAUSE = '&6' -- e.g. "IN ('TEMP1','TEMP2')" or "LIKE ('%')"
...
BEGIN
  ...
  BEGIN
    ...
    WHILE K IS NOT NULL LOOP
      ...
      -- This ensures we only process what the user asked for.
      BEGIN
  ...
  /* -------------------------------------------------------------------- */
  DBMS_OUTPUT.PUT_LINE('SUMMARY: Total rows processed='||V_TOTAL_ROWS);
  -- ROLLBACK; -- TEST MODE
  COMMIT; -- PRODUCTION MODE
