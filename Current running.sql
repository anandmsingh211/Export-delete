/* Formatted on 2/10/2026 3:49:28 PM (QP5 v5.417) */
DECLARE
    /* 1) CONTEXT VARIABLES */
    V_CURRENT_DB       VARCHAR2 (30);
    V_TARGET_DB        VARCHAR2 (30) := 'FSSNDX';

    -- Date Conversion (MM/DD/YYYY -> TIMESTAMP)
    V_START_TS         TIMESTAMP := TO_TIMESTAMP ('05/01/2023', 'MM/DD/YYYY');
    V_END_TS           TIMESTAMP
        := TO_TIMESTAMP ('03/01/2024', 'MM/DD/YYYY') + INTERVAL '1' DAY;

    /* Filter Variables for "Select Template" Logic */
    V_EXPORT_ALL_OPT   CHAR (1) := TRIM ('2');
    V_RAW_LIST         VARCHAR2 (4000)
        := UPPER (TRIM ('APPYMTXX,AP_PYMNT,ARPEND,GL_LEDXX'));

    TYPE T_TEMPLATE_LIST IS TABLE OF BOOLEAN
    ...
    TYPE T_COUNT_MAP IS TABLE OF PLS_INTEGER
    ...
    TYPE T_OUT_BUFFER IS TABLE OF VARCHAR2 (4000)
    ...
    FUNCTION MK_KEY (P_TEMPLATE VARCHAR2, P_TABLE VARCHAR2)
        RETURN VARCHAR2
    IS
    BEGIN
...
BEGIN
    ...
    IF UPPER (V_CURRENT_DB) <> UPPER (V_TARGET_DB)
    THEN
        DBMS_OUTPUT.PUT_LINE (
    ...
    BEGIN
        ...
        BEGIN
            SELECT directory_path
             ...
             WHERE directory_name = UPPER ('DATAMIN_LOG_DIR');
        EXCEPTION
            WHEN NO_DATA_FOUND
            THEN
        ...
        UTL_FILE.FGETATTR (UPPER ('DATAMIN_LOG_DIR'),
                           TRIM ('DataMinExports_FSLOG_20251211_230319.out'),
                           V_EXISTS,
                           V_FILE_LEN,
                           V_BLKSIZE);
        ...
        THEN
            RAISE_APPLICATION_ERROR (
                ...
                || 'DATAMIN_LOG_DIR'
                || '/'
                || 'DataMinExports_FSLOG_20251211_230319.out');
        END IF;

        -- Open file
        F_LOG :=
            UTL_FILE.FOPEN (
                UPPER ('DATAMIN_LOG_DIR'),
                TRIM ('DataMinExports_FSLOG_20251211_230319.out'),
                'R',
                32767);
       ...
       <<READ_LOOP>>
        LOOP
            ...
            EXCEPTION
                ...
                THEN
                    ...
                    RAISE_APPLICATION_ERROR (-20014,
                                             'Read error in log file');
            END;

            /* Detect template banner */
            IF REGEXP_LIKE (V_LINE, '^Beginning export of Template\s*:')
            THEN
                V_CUR_TEMPLATE :=
                    REGEXP_SUBSTR (
                        V_LINE,
                ...
                IF V_EXPORT_ALL_OPT = '2' AND V_CUR_TEMPLATE IS NOT NULL
                THEN
                    IF NOT V_FILTER_MAP.EXISTS (V_CUR_TEMPLATE)
                    THEN
            ...
            IF REGEXP_LIKE (V_LINE, '\s(\d+)\s+rows')
            THEN
                V_ROWS :=
                    TO_NUMBER (REGEXP_SUBSTR (V_LINE,
                                              '\s(\d+)\s+rows',
                                              1,
                                              1,
                                              NULL,
            ...
            THEN
                ...
                -- Safe Map Population
                IF V_EXPECTED_LOG.EXISTS (V_KEY)
                THEN
    ...
    BEGIN
        ...
        LOOP
            ...
            FOR BATCH_REC
                IN (  SELECT DISTINCT B.PSARCH_BATCHNUM, B.PSARCH_ID
                        FROM PSARCHBATCH B
                             JOIN PSARCHTEMPOBJ T ON B.PSARCH_ID = T.PSARCH_ID
                             JOIN PSARCHOBJREC A
                                 ON T.PSARCH_OBJECT = A.PSARCH_OBJECT
            ...
            LOOP
                ...
                BEGIN
                    ...
                    IF V_EXP_CNT = V_DEL_CNT AND V_POST_CNT = 0
                    THEN
            ...
            THEN
                ...
                IF V_BATCH_MSGS.COUNT > 0
                THEN
                    FOR i IN V_BATCH_MSGS.FIRST .. V_BATCH_MSGS.LAST
                    LOOP
            ...
            -- NOTE: To commit table-by-table, add the line below.

            ELSE
                -- FAILURE: Mismatch found. Stop everything.
                RAISE_APPLICATION_ERROR (
                    -20031,
                       'EXPORT/DELETE MISMATCH '
                    || K
                    || ' Expected='
                    || V_EXPECTED_TOTAL
                    || ' Deleted='
